...
msg('* {} tests. See {}'.format(len(all), input_file))
msg('* {} test suites. See {}'.format(len(suite), suite_file))
msg('* {} test collections. See {}'.format(len(suite_collection), suite_collection_file))
msg('* {} types. See {}'.format(len(types), type_file))
msg('* {} collections with two types. See {}'.format(len(suite_collection_double_type), suite_collection_double_type_file))

msg('\nTest execution statistics:')
msg('\n* {:>4} tests in total'.format(len(all)))
if len(not_implemented) > 0:
    msg('--> {} test not executed, because features not implemented in INSTANS. See {}'.format(len(not_implemented), not_implemented_file))
...
if len(negative_syntax_parsed) > 0:
    msg('------------> {:>3} negative syntax tests unexpectedly parsed! See {}'.format(len(negative_syntax_parsed), negative_syntax_parsed_file))
...
if len(positive_syntax_not_parsed) > 0:
    msg('-------------> {} positive syntax tests did not parse! See {}'.format(len(positive_syntax_not_parsed), positive_syntax_not_parsed_file))
...
if len(positive_syntax_parsed_not_translated) > 0:
    msg('------------------> {:>3} positive syntax tests parsed, but did not translate! See {}'.format(len(positive_syntax_parsed_not_translated), positive_syntax_parsed_not_translated_file))
...
if len(positive_syntax_parsed_translated_not_initialized) > 0:
    msg('------------------------> {:>3} positive syntax tests parsed and translated, but did not initialize! See {}'.format(len(positive_syntax_parsed_translated_not_initialized), positive_syntax_parsed_translated_not_initialized_file))
...
msg('------> {:>3} query evaluation tests not runnable (not parsed, translated, or initialized)! See {}'.format(len(query_not_runnable), query_not_runnable_file))
...
if len(query_runnable_not_implemented) > 0:
    msg('------------> {:>3} runnable query evaluation tests with non-implemented operations in INSTANS! See {}'.format(len(query_runnable_not_implemented), query_runnable_not_implemented_file))
...
if len(query_runnable_implemented_not_ran) > 0:
    msg('------------------> {:>3} query runnable evaluation tests fully implemented in INSTANS, but dit not run! See {}'.format(len(query_runnable_implemented_not_ran), query_runnable_implemented_not_ran_file))
...
if len(query_runnable_implemented_ran_not_compared) > 0:
    msg('------------------------> {:>3} query runnable evaluation tests fully implemented in INSTANS, ran, but dit not compare! See {}'.format(len(query_runnable_implemented_ran_not_compared), query_runnable_implemented_ran_not_compared_file))
...
msg('\n* {:>4} tests in total'.format(len(all)))
if len(failed) > 0:
    msg('-------> {} tests failed. See {}'.format(len(failed), failed_file))
