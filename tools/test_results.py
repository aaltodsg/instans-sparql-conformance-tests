...
query_runnable_implemented_crashed_file = os.path.join(output_dir, "query-runnable-implemented-crashed.csv")
...
suite = allsanstimes.project('suite').dropDuplicates()
suite_collection = allsanstimes.project('suite','collection').dropDuplicates()
types=allsanstimes.project('type').sort('type',unique=True)
suite_collection_type = allsanstimes.project('type','suite','collection').dropDuplicates()
def csrows(g,rows):
...
not_implemented = allsanstimes.select(lambda r:r['implementedp'] == 'False')
implemented = allsanstimes.select(lambda r:r['implementedp'] == 'True')
syntax = allsanstimes.select(lambda r:r['syntax-test-p'] == 'True')
...
query_runnable_implemented_crashed = query_runnable_implemented.select(lambda r: r['running-crashed-p'] == 'True')
...
failed = allsanstimes.select(lambda r:r['failedp'] == 'True')
not_failed = allsanstimes.select(lambda r:r['failedp'] == 'False')
failed_implemented = allsanstimes.select(lambda r:r['failedp'] == 'True' and r['implementedp'] == 'True')
failed_not_implemented = allsanstimes.select(lambda r:r['failedp'] == 'True' and r['implementedp'] == 'False')

write_results(suite,suite_file)
write_results(suite_collection,suite_collection_file)
write_results(suite_collection_double_type,suite_collection_double_type_file)
write_results(not_implemented,not_implemented_file)
write_results(negative_syntax_parsed,negative_syntax_parsed_file)
write_results(positive_syntax_not_parsed,positive_syntax_not_parsed_file)
write_results(positive_syntax_parsed_not_translated,positive_syntax_parsed_not_translated_file)
write_results(positive_syntax_parsed_translated_not_initialized,positive_syntax_parsed_translated_not_initialized_file)
write_results(query_not_runnable,query_not_runnable_file)
write_results(query_runnable_not_implemented,query_runnable_not_implemented_file)
write_results(query_runnable_implemented_crashed,query_runnable_implemented_crashed_file)
write_results(query_runnable_implemented_not_ran,query_runnable_implemented_not_ran_file)
write_results(query_runnable_implemented_ran_different_semantics,query_runnable_implemented_ran_different_semantics_file)
write_results(query_runnable_implemented_ran_not_compared,query_runnable_implemented_ran_not_compared_file)
write_results(not_failed,not_failed_file)
write_results(failed,failed_file)
write_results(failed_implemented,failed_implemented_file)
write_results(failed_not_implemented,failed_not_implemented_file)

msg('\nTest suite, collection, and type statistics:')
msg('* {} tests.{}'.format(len(allsanstimes), see(input_file)))
...
msg('\n* {:>4} tests in total'.format(len(allsanstimes)))
...
check_counts(implemented, not_implemented, allsanstimes, 6, 'tests implemented', 'tests not implemented', 'allsanstimes tests', implemented_undefined_file)
if len(failed) > 0:
...
if len(query_runnable_implemented_not_ran) > 0:
    ...
    msg('                    All query runnable evaluation tests fully implemented in INSTANS ran')
    
if len(query_runnable_implemented_crashed) > 0:
    msg('------------------> {:>3} query runnable evaluation tests fully implemented in INSTANS crashed!{}'.format(len(query_runnable_implemented_crashed), see(query_runnable_implemented_crashed_file)))
else:
    msg('                    No query runnable tests crashed')
